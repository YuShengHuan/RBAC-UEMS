<template>
   ...
   >
     <template #topOperate="{pageParameter}">
       <el-button :icon="Plus" type="primary" @click="handleCreate" v-permission="'user:create'">创建</el-button>
       <el-button :icon="TopRight" type="success" @click="handleImport" v-permission="'user:batchInsert'">导入</el-button>
       <el-button :icon="TopLeft" type="success" @click="handleExport(pageParameter)" v-permission="'user:export'">导出</el-button>
       <el-button :icon="Delete" type="danger" @click="handleBatchDelete(pageParameter)" v-permission="'user:batchDelete'">批量删除</el-button>
     </template>
     <template #rowOperate="{index,row}">
         <el-button :icon="Edit" type="primary" @click="handleEdit(row)" v-permission="'user:update'">编辑</el-button>
         <el-button :icon="Delete" type="danger" @click="handleDelete(row)" v-permission="'user:delete'">删除</el-button>
       <el-button :icon="RefreshLeft" type="success" @click="handleResetPassword(row)" v-permission="'user:resetPwd'">重置密码</el-button>
     </template>
   </FreedomDataManage>
   <AdminSysUserForm
       ref="formViewRef"
...
import  AdminSysUserForm from '../user/form/AdminSysUserForm.vue'
...
import  {tableDataConfig,filterFormConfig,baseApi,importFormConfig} from "../../../config/module/AdminSysUserConfig"
...
const handleResetPassword=async (row)=>{
  try {
    const res=await request.put(`${baseApi}/reset-password/${row.id}`)
    if(res.status===200){
      await freedomDataManageRef.value.initData()
      await ElMessageBox.alert(res.data, {confirmButtonText: '确定'})
    }
  }catch (e) {
    console.log(e)
  }
